...
#define _NosepokeImpureShared_h
...
inline void RunNosepokeImpureMachine(NosepokeImpureState &st, int unstrProb){
  ...
  if ((epochMillis() - st.sessionStartMs) >= SESSION_LIMIT_MS){ stateStop = 1; }
      ...
      if (Nosepoke1DI.isOn() || Nosepoke2DI.isOn()){
        ...
        for (int i=0;i<=1;++i){ if (recordNosepoke[i]==1) n = whichnosepoke[i]; }
        ...
        while ((millis() - lickWindowStart) <= 5000){
          HouseKeeping();
          if (stateStop || checkSMStateStopGeneric(st)) { lickDetected = false; break; }
          if (LickDI.isOn()){
            lickDetected = true;
            LickDI.clear();
...
inline void NosepokeImpureSharedMachine(){
  RunNosepokeImpureMachine(g_nosepokeImpureState, g_unstructuredProbSetting);
}
